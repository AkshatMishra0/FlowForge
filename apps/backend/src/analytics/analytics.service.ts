...
export class AnalyticsService {
  ...
  async getWeeklyRevenueTrend(
    businessId: string,
    weeks: number = 8,
  ): Promise<{ week: string; revenue: number; invoiceCount: number }[]> {
    const startDate = new Date();
    ...
    const weeklyData: { [key: string]: { revenue: number; count: number } } = {};
    ...
    invoices.forEach((inv) => {
      if (inv.paidAt) {
        ...
        weeklyData[weekKey].revenue += inv.totalAmount;
